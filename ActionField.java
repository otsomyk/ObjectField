...
public class ActionField extends JPanel {
	...
	public void processMove(AbstractTank abstractTank) throws Exception {
		...
		int direction = abstractTank.getDirection().getId();
		int step = 4;
		if ((direction == 1 && abstractTank.getY() == 0)
				|| (direction == 2 && abstractTank.getY() >= 512)
				|| (direction == 3 && abstractTank.getX() == 0)
				|| (direction == 4 && abstractTank.getX() >= 512)) {
			return;
		} else
